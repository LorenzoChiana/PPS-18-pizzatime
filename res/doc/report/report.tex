...
\comment{            
        ...
        \section{Requisiti non funzionali}\label{sec:requirements:notfunctional}
           	\input{3-requisiti/nonFunctional.tex}
        \section{Requisiti implementativi}\label{sec:requirements:implementative}
            \input{3-requisiti/implementative.tex}
    ...
    \chapter{Implementazione}\label{ch:implementation}
        ...
        \section{Lorenzo Chiana}\label{sec:chiana}
            \input{6-implementazione/chiana/chiana.tex}
    
        %\clearpage
        
        \section{Meshua Galassi}\label{sec:galassi}
            \input{6-implementazione/galassi/intro.tex}
            \subsection{Sviluppo}\label{subsec:galassi:sviluup}
                \input{6-implementazione/galassi/sviluppo.tex}
            \subsection{Refactor}\label{subsec:galassi:refactor}
                \input{6-implementazione/galassi/refactor.tex}
        \clearpage
    
        \section{Giada Gibertoni}\label{sec:gibertoni}
            \input{6-implementazione/gibertoni/gibertoni.tex}
            \subsection{GameManager}
            \input{6-implementazione/gibertoni/controller.tex}
            \subsection{GameView}
            \input{6-implementazione/gibertoni/view.tex}
            \subsection{GameLogic}
            \input{6-implementazione/gibertoni/model.tex}
            \subsection{Utilities}
            \input{6-implementazione/gibertoni/utilities}
        
        %\clearpage
        
        \section{Giacomo Pasini}\label{sec:pasini}
            \input{6-implementazione/pasini/intro.tex}
            \subsection{Entity}\label{subsec:pasini:entity}
                \input{6-implementazione/pasini/entity.tex}
            \subsection{Arena}\label{subsec:pasini:arena}
                \input{6-implementazione/pasini/arena.tex}
            \subsection{Utilities}\label{subsec:pasini:utilities}
                \input{6-implementazione/pasini/utilities.tex}
            \subsection{Testing}\label{subsec:pasini:testing}
                \input{6-implementazione/pasini/testing.tex}
        
        \section{Pair Programming - Meshua Galassi, Giada Gibertoni}\label{sec:pairprogramming}
            \input{6-implementazione/pairprogramming/galassigibertoni.tex}
        \section{Pair Programming - Lorenzo Chiana, Giacomo Pasini}\label{sec:pairprogramming}
            \input{report/6-implementazione/pairprogramming/chianapasini.tex}
            
    \clearpage

    \chapter{Retrospettiva e commenti finali}\label{ch:retrospective}
        \input{7-retrospettiva/retrospective.tex}
